...
const SUPABASE_CONFIGURED = SUPABASE_URL && SUPABASE_ANON_KEY && !SUPABASE_URL.includes("YOUR-PROJECT") && !SUPABASE_ANON_KEY.includes("YOUR_ANON_KEY") && supabaseClient;
...
function Sidebar({ user, activeTab, onTab, onLogout, badges }) {
  ...
  return (
    <div style={{width:230,background:T.sb,display:"flex",flexDirection:"column",flexShrink:0,height:"100vh",position:"sticky",top:0}}>
      <div style={{padding:"18px 18px 16px",borderBottom:`1px solid ${T.sbBorder}`}}><div style={{background:"#fff",borderRadius:12,padding:"10px 14px",display:"inline-block"}}>
        <img src={window.MOGA_LOGO} alt="MOGA" style={{width:130,display:"block"}}/>
        </div>
        <div style={{fontSize:9,fontWeight:600,color:"rgba(255,255,255,0.3)",letterSpacing:"0.15em",marginTop:8,paddingLeft:4}}>VETERINARIA</div>
      </div>
...
function LoginScreen({ users, onLogin, supabaseError }) {
  useGlobalStyle();
        ...
        <div>
          <div style={{marginBottom:60}}>
            <div style={{background:"#fff",borderRadius:12,padding:"10px 16px",display:"inline-block"}}>
              <img src={window.MOGA_LOGO} alt="MOGA" style={{width:140,display:"block"}}/>
            </div>
          </div>
...
function App() {
  ...
  useEffect(()=>{
    ...
    const load = async () => {
      try {
        const tableNames = ["users","pets","records","grooming","vaccines","payments"];
        const results = await Promise.all(tableNames.map(name => supabaseClient.from(name).select("*")));
        const errors = results.map(r => r.error).filter(Boolean);
        if(errors.length) throw errors[0];
        setUsers(results[0].data || []);
        setPets(results[1].data || []);
        setRecords(results[2].data || []);
        setGrooming(results[3].data || []);
        setVaccines(results[4].data || []);
        setPayments(results[5].data || []);
      } catch (err) {
        console.warn("Supabase no disponible, usando datos demo:", err.message);
      }
    };
    load();
  }, []);

  // ─── MUTATION HANDLERS (Supabase + fallback local) ───────────
  const addPet = async p => {
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("pets").insert([p]).select().single();
      if (!error && data) { setPets(prev=>[...prev,data]); return; }
    }
    setPets(prev=>[...prev,{...p,id:Date.now()}]);
  };

  const addRecord = async r => {
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("records").insert([r]).select().single();
      if (!error && data) { setRecords(prev=>[...prev,data]); return; }
    }
    setRecords(prev=>[...prev,{...r,id:Date.now()}]);
  };

  const addGrooming = async g => {
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("grooming").insert([g]).select().single();
      if (!error && data) { setGrooming(prev=>[...prev,data]); return; }
    }
    setGrooming(prev=>[...prev,{...g,id:Date.now()}]);
  };

  const updateGroomingStatus = async (id, s) => {
    if (SUPABASE_CONFIGURED) {
      const { error } = await supabaseClient.from("grooming").update({status:s}).eq("id",id);
      if (!error) { setGrooming(prev=>prev.map(g=>g.id===id?{...g,status:s}:g)); return; }
    }
    setGrooming(prev=>prev.map(g=>g.id===id?{...g,status:s}:g));
  };

  const addUser = async u => {
    const rec = {...u, role:"client"};
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("users").insert([rec]).select().single();
      if (!error && data) { setUsers(prev=>[...prev,data]); return; }
    }
    setUsers(prev=>[...prev,{...rec,id:Date.now()}]);
  };

  const addVaccine = async v => {
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("vaccines").insert([v]).select().single();
      if (!error && data) { setVaccines(prev=>[...prev,data]); return; }
    }
    setVaccines(prev=>[...prev,{...v,id:Date.now()}]);
  };

  const addPayment = async p => {
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("payments").insert([p]).select().single();
      if (!error && data) { setPayments(prev=>[...prev,data]); return; }
    }
    setPayments(prev=>[...prev,{...p,id:Date.now()}]);
  };

  const markPaid = async (id, m) => {
    if (SUPABASE_CONFIGURED) {
      const { error } = await supabaseClient.from("payments").update({status:"pagado",method:m}).eq("id",id);
      if (!error) { setPayments(prev=>prev.map(p=>p.id===id?{...p,status:"pagado",method:m}:p)); return; }
    }
    setPayments(prev=>prev.map(p=>p.id===id?{...p,status:"pagado",method:m}:p));
  };

  const bookGrooming = async (appt, clientId) => {
    const rec = {...appt, clientId};
    if (SUPABASE_CONFIGURED) {
      const { data, error } = await supabaseClient.from("grooming").insert([rec]).select().single();
      if (!error && data) { setGrooming(prev=>[...prev,data]); return; }
    }
    setGrooming(prev=>[...prev,{...rec,id:Date.now()}]);
  };

  if(!currentUser) return <LoginScreen users={users} onLogin={setCurrentUser} supabaseError={supabaseError}/>;
  if(currentUser.role==="client"){
    ...
    return <ClientPortal
      ...
      onBookGrooming={appt=>bookGrooming(appt, currentUser.id)}
    />;
  }
  return <AdminDashboard
    ...
    onAddGrooming={addGrooming}
    onUpdateGroomingStatus={updateGroomingStatus}
    onAddUser={addUser}
